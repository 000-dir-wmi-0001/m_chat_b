...
export class TextChatGateway implements OnGatewayConnection, OnGatewayDisconnect {
  ...
  handleCreateRoom(@ConnectedSocket() client: Socket) {
    ...
    console.log('Text Chat - Creating room for client:', client.id, 'IP:', ip);
    ...
    client.join(code);
    console.log('Text Chat - Room created with code:', code, 'Creator:', client.id);
    return { code };
  }
  ...
  handleJoinRoom(@MessageBody() data: { code: string }, @ConnectedSocket() client: Socket) {
    const ip = this.getClientIP(client);
    console.log('Text Chat - Client', client.id, 'trying to join room:', data.code, 'IP:', ip);
    
    const room = this.textRooms[data.code];
    if (!room) {
      if (!this.checkRateLimit(ip, 'join')) {
        console.log('Rate limit exceeded for failed join attempt, IP:', ip);
        return { error: 'Too many failed attempts. Please try again later.' };
      }
      console.log('Text Chat - Room not found:', data.code);
      return { error: 'Room not found' };
    }
    
    room.users.push(client.id);
    client.join(data.code);
    console.log('Text Chat - User joined room:', data.code, 'Users:', room.users.length);
  ...
  handleSendFile(
    ...
    try {
      ...
      console.log('Text Chat - Sending file:', data.file.name, 'Size:', data.file.size);
  ...
  handleLeaveRoom(@MessageBody() data: { code: string }, @ConnectedSocket() client: Socket) {
    console.log('Text Chat - Client', client.id, 'leaving room:', data.code);
    const room = this.textRooms[data.code];
